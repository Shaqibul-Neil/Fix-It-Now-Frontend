...
import { CATEGORY_DETAILS } from "../dependencies/constants/category.dummy";
...
const CategoryDetailsPage = ({ slug }: { slug: string }) => {
  ...
  const heroStats = [
    { id: "technicians", value: category.technicianCount, label: "Verified technicians" },
    { id: "services", value: category.serviceCount, label: "Live services" },
    { id: "jobs", value: category.completedJobs.toLocaleString(), label: "Jobs completed" },
    { id: "rating", value: category.averageRating, label: "Average rating" },
    { id: "response", value: `${category.responseMinutes} min`, label: "Median response" },
  ];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 xl:px-8">
        <ScrollReveal className="grid gap-10 border-b border-project-border py-16 lg:grid-cols-[1fr_1.15fr] lg:gap-16">
          <PageHeader
            ...
            description={`Rates in this category run from ${formatMoney(category.priceRange.min)} to ${formatMoney(category.priceRange.max)} depending on the job.`}
            action={{
